document.addEventListener("DOMContentLoaded", () => {
  ...
  if(msgBox) msgBox.innerText=`"${dailyMessages[dayIndex % dailyMessages.length]}"`;
...
`;
  ...
  if(mainPhoto && photos.length>0) mainPhoto.src=photos[Math.floor(Math.random()*photos.length)].src;
  ...
  const openMessages=[
      ...
      "Can't Sleep: Close your eyes, I’m thinking of you 🌙\nSweet dreams soon 💫",
      "Need Motivation: You can do anything 💪\nI believe in you 🌸",
      "Thinking About Us: I love thinking about us ❤️\nWe have so many memories 💕"
  ];
  const openText=document.getElementById("openText");

  window.openMsg=function(index){
      if(openText) openText.innerText=openMessages[index].split(": ")[1];
  }
