...
if (isHome) {
  ...
  menuToggle.addEventListener("click", () => {
    menu.style.left = menu.style.left === "0px" ? "-50%" : "0px";
  });

  // Scroll buttons inside menu
  function scrollToSection(sectionId) {
    const target = document.getElementById(sectionId);
    if (target) {
      target.scrollIntoView({ behavior: "smooth" });
      menu.style.left = "-50%"; // close menu after click
    }
  }
  ...
  let currentMusic = 0;
  musicList.forEach(m => m.loop = false);
  ...
  window.toggleMusic = function (track) {
    musicList.forEach(m => m.pause());
    currentMusic = track === "apocalypse" ? 0 : 1;
    musicList[currentMusic].play();
    musicList[currentMusic].onended = playNextMusic;
  };
