document.addEventListener("DOMContentLoaded", () => {
    ...
    if (isLogin) {
        ...
        // Falling text background for login
        const lines = [
    ...
    function updateGMGN() {
        ...
        if (gmgn) gmgn.innerText = hour >= 5 && hour < 18 ? "Good Morning Rita ☀️" : "Good Night Rita 🌙";
    }
    updateGMGN();
    ...
    const msgTitleBox = document.createElement("div");
    msgTitleBox.id = "msgOfDayTitleBox";
    msgTitleBox.innerText = "Message of the Day 💗";
    if (msgBox) {
        msgBox.parentNode.insertBefore(msgTitleBox, msgBox);
    }
    const dailyMessages = [
        ...
        "You are my happiness 💗", "My heart beats faster when I think of you 💓",
        "You’re my sweet escape 🍬", "Life is better with you 💞", "Forever in my thoughts 💌",
        "I love the way you laugh 😍", "You’re my everything 🌹", "I can’t stop smiling thinking of you 😊",
        "You make ordinary days magical ✨", "Just wanted to say I adore you 💕",
        "Every time I see you, I fall a little more ❤️", "You are my sunshine ☀️",
        "I love our little moments 💖", "Being with you feels like home 🏡",
        "You are my forever 💗"
    ];
    if (msgBox) {
        const todayKey = "msgOfTheDay_" + new Date().toDateString();
        let todayMsg = localStorage.getItem(todayKey);
        if (!todayMsg) {
            todayMsg = dailyMessages[Math.floor(Math.random() * dailyMessages.length)];
            localStorage.setItem(todayKey, todayMsg);
        }
        msgBox.innerText = todayMsg;
    }

    // ---------- LOVE LETTER ----------
    const letterBox = document.getElementById("loveLetterBox");
    const letterTitleBox = document.createElement("div");
    letterTitleBox.id = "loveLetterTitleBox";
    letterTitleBox.innerText = "A Little Letter for You 💌";
    if (letterBox) {
        letterBox.parentNode.insertBefore(letterTitleBox, letterBox);
        letterBox.innerHTML = `
Rita,
...
that someone out there is thinking about you.
    ...
    const openMessages = {
        "Sad": [
            "I’m here if you need me 💖", "You are stronger than you think 🌟",
            "Sending you hugs 🤗", "Everything will be okay 💞"
        ],
        "Miss Me": [
            "I miss you too 😘", "Wish I could hug you right now 🤗",
            "Counting the moments till we meet ❤️", "I can’t wait to see you 😍"
        ],
        "Can't Sleep": [
            "Close your eyes, I’m thinking of you 🌙", "Sweet dreams soon 💫",
            "Imagine us together 😘", "Let go of worries and sleep peacefully 💤"
        ],
        "Need Motivation": [
            "You can do it, I believe in you 💪", "Every day is a new chance ✨",
            "Your dreams are worth it 🌟", "Keep pushing forward ❤️"
        ],
        "Thinking About Us": [
            "I think about us always 💕", "You make my heart smile ❤️",
            "Remember our happy times 💖", "You’re always in my heart 💗"
        ]
    };
    ...
    window.toggleMusic = function(name) {
        if(currentMusic !== null) musicList[currentMusic].pause();
